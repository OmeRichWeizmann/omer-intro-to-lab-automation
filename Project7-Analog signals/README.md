...
- what happens when using 30Hz instead of 50Hz for the PWM? -----A: when using 30 Hz it seems that the LED is blinking, the frequency is lower so we can detect the Off times.
...
- first check in adalm the range of the mapped values. What frequency is the Servo package using? -----A: THe servo packag is using 50 Hz
...
- How does the range of the servo motor compare to the range of the rotary? Use the scope. ----A: The duty cycle of the Servo reach only to 12%, the range of the rotery is much larger. 
- is the range of our servo the same as the range of the package? change the range of values to the servo accordingly. ---A: No, it seems that while the package allow up to 180 deg, the servo does'nt follow it all the way.  
